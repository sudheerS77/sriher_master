...
import {
  getProject,
  deleteProject,
} from "../../../Redux/Reducer/Projects/project.action";
            ...
            {projects?.length > 0 ? (
              projects?.map((row) => (
                <TableRow key={row.id} className="hover:bg-gray-100">
                  ...
                  <TableCell className="tableCell">
                    <div className="flex items-center">
                      <img
                        src={row.image}
                        alt=""
                        className="w-44 h-20 rounded-md mr-10 fit"
                      />
                      {row.projectName}
                    </div>
                  </TableCell>
                  <TableCell
                    className="tableCell w-96"
                    style={{ height: "20px" }}
                  >
                  ...
                  <TableCell className="tableCell">
                    <Link to={`/admin/projects/${row._id}`} id="edit_label">
                      <span
